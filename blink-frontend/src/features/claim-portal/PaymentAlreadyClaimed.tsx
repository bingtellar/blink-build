...
interface PaymentAlreadyClaimedProps {
  paymentData: EscrowPayment; 
  onClose?: () => void;
  onLearnMore?: () => void;
}

export const PaymentAlreadyClaimed = ({ paymentData, onClose, onLearnMore }: PaymentAlreadyClaimedProps) => {
  
  const claimDateStr = React.useMemo(() => {
    if (!paymentData.timeline) return null;
    
    const completionEvent = paymentData.timeline.find(
      e => e.state === "claim_completed" || e.state === "completed" || e.state === "succeeded"
    );
    
    if (completionEvent) {
      return new Date(completionEvent.timestamp).toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric',
        year: 'numeric',
        hour: 'numeric',
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 sm:px-6 w-full max-w-[560px] mx-auto pb-12">
        <div className="bg-white w-full rounded-[12px] border border-black py-16 px-6 sm:py-20 sm:px-16 shadow-[0px_4px_24px_rgba(0,0,0,0.02)] text-center">
          ...
          <h2 className="text-[13px] min-[375px]:text-[14px] sm:text-[20px] font-semibold text-[#1A1A1A] mb-3 tracking-tighter whitespace-nowrap sm:whitespace-normal">
            Oops... This {paymentData.currency?.toUpperCase().includes("USD") ? "$" : ""}{paymentData.amount.toLocaleString()} Blink has already been claimed
          </h2>
          ...
          {claimDateStr ? (
            <div className="flex items-center justify-center gap-2 mb-8 text-[#6B7280] bg-[#F9FAFB] border border-[#E5E7EB] py-1.5 px-4 rounded-full w-max mx-auto shadow-sm">
              <Calendar size={14} />
              <span className="text-[12px] font-medium tracking-wide">Claimed on {claimDateStr}</span>
            </div>
          ) : (
            <div className="mb-8" /> 
          )}
