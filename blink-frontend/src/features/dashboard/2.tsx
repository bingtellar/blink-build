    ...
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center sm:p-4">
      ...
      <div className="relative bg-white w-full sm:max-w-[440px] rounded-t-[24px] sm:rounded-[32px] sm:rounded-b-[32px] overflow-hidden shadow-2xl animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-300 flex flex-col max-h-[96dvh] sm:max-h-[90vh] pb-6 sm:pb-0 h-[90dvh] sm:h-[680px]">
        ...
        <div className="relative bg-white flex-1 overflow-y-auto px-6 py-8 flex flex-col items-center">
          ...
          <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-5 ${isDeposit ? 'bg-[#E5F7ED] text-[#3BA66A]' : 'bg-[#F5F5F4] text-[#1A1A1A]'}`}>
...
export const TransactionHistory = ({ transactions, accountId }: TransactionHistoryProps) => {
  ...
  const isEmpty = Object.keys(groupedTransactions).length === 0;
      ...
      <div className="hidden md:block mb-6">
        <div className="flex flex-col gap-8">
          ...
          <div className="relative flex items-center gap-6 text-[14px]">
            {/* The full-width thin gray line under tabs */}
        ...
        {isEmpty ? (
          ...
          <div className="space-y-8 mt-4 md:mt-6">
            {Object.keys(groupedTransactions).map((dateKey) => (
              <div key={dateKey}>
                <h3 className="text-[12px] font-bold text-[#A3A3A3] uppercase tracking-wider mb-4 border-b border-[#F5F5F4] pb-2 md:border-none md:pb-0">
                  {dateKey}
                </h3>
                <div className="space-y-2">
                  {groupedTransactions[dateKey].map((tx) => (
                    <button 
                      ...
                      className="w-full bg-white hover:bg-[#F9F9F9] rounded-[20px] md:rounded-[12px] p-3 flex items-center justify-between transition-colors text-left group"
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-11 h-11 rounded-full flex items-center justify-center shrink-0 transition-colors ${tx.type === 'deposit' ? 'bg-[#E5F7ED] text-[#3BA66A] group-hover:bg-[#D1F0DE]' : 'bg-[#F5F5F4] text-[#1A1A1A] group-hover:bg-[#E8E8E8]'}`}>
                          {tx.type === 'deposit' ? <ArrowDownToLine size={20} /> : <ArrowUpRight size={20} />}
                        </div>
                        <div>
                          <h4 className="text-[14px] font-bold text-[#1A1A1A] leading-snug">{tx.description}</h4>
                          <div className="flex items-center gap-1.5 mt-1">
                            {tx.status === 'pending' && <Clock size={12} className="text-[#D97706]" />}
                            {tx.status === 'completed' && <Check size={12} className="text-[#3BA66A]" />}
                            {tx.status === 'failed' && <AlertCircle size={12} className="text-red-500" />}
                            <p className={`text-[11px] font-medium capitalize tracking-wide ${tx.status === 'pending' ? 'text-[#D97706]' : tx.status === 'failed' ? 'text-red-500' : 'text-[#757575]'}`}>
                              {tx.status} • {new Date(tx.date).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="text-right">
                        <h4 className={`text-[15px] font-bold ${tx.type === 'deposit' ? 'text-[#3BA66A]' : 'text-[#1A1A1A]'}`}>
                          {tx.type === 'deposit' ? '+' : '-'}{tx.amount.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </h4>
                        <p className="text-[11px] text-[#A3A3A3] mt-1 font-semibold">USDC</p>
                      </div>
                    </button>
                  ))}
                </div>
