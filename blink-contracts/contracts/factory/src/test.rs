...
use crate::{BlinkFactory, BlinkFactoryClient};
use blink_types::{AgreementType, EscrowConfig, YieldPolicy};
...
fn test_factory_deployment_and_claim_with_dynamic_fee() {
    ...
    // 🌟 ADDED: Initialize the Factory with the true treasury address so it doesn't panic
    factory.init(&platform_treasury);

    let vault_wasm_hash = env
        .deployer()
        .upload_contract_wasm(Bytes::from_slice(&env, vault_wasm::WASM));

    // 🟢 FIX: Generate the secret as dynamic Bytes to match the Vault's expectation
    let secret = Bytes::from_slice(&env, &[1; 32]);
    ...
    vault_client.claim(&secret, &recipient, &1000); // Pass the 1000 claim amount
